...
namespace Modules\Train\Controllers;
...
class ManageTrainSeatController extends Controller
{
    protected $flight;
    ...
    public function __construct()
    {
        ...
        $this->flightSeat = TrainSeat::class;
    }
    ...
    public function index(Request $request,$flight_id)
    {
        ...
        $data = [
            ...
            'breadcrumbs'        => [
                ...
                [
                    'name' => __('Train: :name',['name'=>$this->currentFlight->title]),
                    'url'  => route('flight.vendor.edit',[$this->currentFlight->id])
                ],
                [
                    'name'  => __('All Train seats'),
                    'class' => 'active'
                ],
            ],
        ...
        return view('Train::frontend.manageFlight.seat.index', $data);
    }
    ...
    public function create($flight_id)
    {
        ...
        $data = [
            ...
            'page_title'         => __("Create Train seat"),
            'currentFlight'=>$this->currentFlight
        ];
        return view('Train::frontend.manageFlight.seat.detail', $data);
    }
    ...
    public function store( Request $request, $flight_id,$id ){
        ...
        $validator = Validator::make($request->all(), [
            'seat_type'=>[
                'required',
                Rule::unique(TrainSeat::getTableName())->where(function ($query)use($flight_id){
        ...
        if ($res) {
            if($id > 0 ){
                return redirect()->back()->with('success',  __('Train seat updated') );
            }else{
                return redirect(route('flight.vendor.seat.edit',['flight_id'=>$flight_id,'id'=>$row->id]))->with('success', __('Train seat created') );
            }
        }
    }
