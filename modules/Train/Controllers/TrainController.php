...
class TrainController extends Controller
{
    ...
    {
        $this->trainClass = TrainModel::class;
    ...
    public function index(Request $request)
    {
        ...
        $list = call_user_func([$this->trainClass,'search'],$request);
        $markers = [];
        if (!empty($list)) {
            foreach ($list as $row) {
                $markers[] = [
                    ...
                    "infobox" => view('Train::frontend.layouts.search.loop-grid', ['row' => $row,'disable_lazyload'=>1,'wrap_class'=>'infobox-item'])->render(),
        ...
        $data = [
            ...
            'seatType'           => TrainSeat::get(),
            'flight_min_max_price' => $this->trainClass::getMinMaxPrice(),
            'markers'            => $markers,
            "blank" => setting_item('search_open_tab') == "current_tab" ? 0 : 1 ,
            "seo_meta"           => $this->trainClass::getSeoMetaForPageList()
        ];
        ...
        if ($is_ajax) {
            return $this->sendSuccess([
                'html'    => view('Train::frontend.layouts.search-map.list-item', $data)->render(),
        ...
        if ($layout == "map") {
            ...
            return view('Train::frontend.search-map', $data);
        }
        return view('Train::frontend.search', $data);
    }

    public function getData(Request $request,$id){
        $row = $this->trainClass::with(['TrainSeat.seatType','airportFrom','airportTo','airline','bookingPassengers'])->find($id);
        if ( empty($row)) {
