...
use Modules\Train\Factories\TrainFactory;
...
class TrainModel extends BaseModel
{
    ...
    use CapturesService;
    use HasFactory;
    protected $table                              = 'bravo_train';
    public    $type                               = 'train';
    public    $checkout_booking_detail_file       = 'Train::frontend.booking.detail';
    public    $checkout_booking_detail_modal_file = 'Train::frontend.booking.detail-modal';
    public    $set_paid_modal_file                = 'Train::frontend.booking.set-paid-modal';
    public    $email_new_booking_file             = 'Train::emails.new_booking_detail';
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->termClass = TrainTerm::class;
    }

    public static function getModelName()
    {
        return __("TrainModel");
    }
    ...
    public function getPriceInRanges($start_date, $end_date)
    {
        ...
        $datesRaw = $this->TrainDateClass::getDatesInRanges($start_date, $end_date, $this->id);
        $dates = [];
        if (!empty($datesRaw)) {
    ...
    {
        $min = TrainSeat::select(['price', 'train_id'])->whereHas('TrainModel', function (Builder $query) {
            $query->where('status', 'publish');
        })->min('price');
        $max = TrainSeat::select(['price', 'train_id'])->whereHas('TrainModel', function (Builder $query) {
            $query->where('status', 'publish');
        })->max('price');
        return [
            $min ?? 0,
    ...
    public static function search(Request $request)
    {
        ...
        if (!empty($price_range = $request->query('price_range'))) {
            ...
            $model_flight->whereHas('TrainSeat');
        }
        ...
        if (!empty($request->seat_type)) {
            ...
            if (!empty($argv)) {
                $model_flight->whereHas('TrainSeat', function (Builder $builder) use ($argv) {
        ...
        $model_flight->with(['TrainSeat', 'airportFrom', 'airportTo', 'airline']);
    ...
    public static function searchCustom(Request $request)
    {
        ...
        return $model_Flight->with('TrainSeat', 'airportFrom', 'airportTo', 'airline')->paginate($limit);
    }
...
//    new module flight
    ...
    {
        return $this->hasOne(Stations::class, 'id', 'airport_from')->withDefault();
    }

    public function airportTo()
    {
        return $this->hasOne(Stations::class, 'id', 'airport_to')->withDefault();
    }

    public function airline()
    {
        return $this->hasOne(CompanyTrain::class, 'id', 'airline_id')->withDefault();
    }

    public function flightSeat()
    {
        return $this->hasMany(TrainSeat::class, 'train_id')->orderBy('price');
    }
