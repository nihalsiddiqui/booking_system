        ...
        <div class="row">
            ...
            <div class="col-md-8">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-left">
                        @if(!empty($rows))
                            <form method="post" action="{{url('admin/module/train/attribute/editTermBulk')}}" class="filter-form filter-form-left d-flex justify-content-start">
                                {{csrf_field()}}
                    ...
                    <div class="col-left">
                        <form method="get" action="{{url('/admin/module/train/attribute/terms/'.$attr->id)}} " class="filter-form filter-form-right d-flex justify-content-end" role="search">
                                ...
                                @if(count($rows) > 0)
                                    @foreach ($rows as $row)
                                        <tr>
                                            ...
                                            <td class="title">
                                                <a href="{{route('train.admin.attribute.term.edit',['id'=>$row->id])}}">{{$row->name}}</a>
                                            </td>
                                            <td>{{ display_date($row->updated_at)}}</td>
                                            <td><a class="btn btn-primary btn-sm" href="{{route('train.admin.attribute.term.edit',['id'=>$row->id])}}"><i class="fa fa-edit"></i> {{__('Edit')}}</a></td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
